...
- `02_merge_alt_columns.py`: This merges the computed flight altitude into the other metadata. It can be run with the `geograypher` enviornment.
...
- `12_create_train_val_split.py`: This determines train-val split at the plot level that approximately balances the species proportion between the two. This can be run with the `geograypher` environment.
