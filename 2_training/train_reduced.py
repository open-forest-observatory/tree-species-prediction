...
def gradsel_reduce(temp_model, base_dataset, reduction_ratio, subbatch_size, device):
    # gather gradients of Discriminator on real data (since that's what we are subsetting)
    grad_mat, grad_sum, subbatch_idxs = gather_discriminator_gradients(
        temp_model, base_dataset, device, subbatch_size, temp_model.batch_size
    )
    ...
    print(
        f"Chose: {len(new_subbatch_idxs)} subbatches ({len(new_subbatch_idxs) * subbatch_size}) out of {len(base_dataset)}"
    )
...
def train_data_reduction(
    ...
    assert (
        int(reduction_ratio * len(base_dataset)) >= 1
    ), f"ERROR: Reduction ratio too small for dataset size!"
    print(
        f"Will select subsets every {selection_interval} epochs, warm start: {num_warm_start_epochs}."
    )

    current_indices = list(range(len(base_dataset)))
    current_subset = Subset(base_dataset, current_indices)
    previous_subset = Subset(
        base_dataset, current_indices
    )  # used to compare current and previous selections
    ...
    for epoch_idx in range(temp_model.cur_epoch, model_config.epochs):
        ...
        if (
            epoch_num
            <= model_config.epochs  # if we haven't trained for the full spec'd num epochs and,
            and epoch_num
            >= num_warm_start_epochs  # if we have trained enough on the full data and,
            and (epoch_num - num_warm_start_epochs) % selection_interval == 0
        ):  # we are at a selection interval epoch,
            ...
            subset_sel_time = (
                time.time() - t0_subset
            )  # track time for selecting subset of data

        else:
            subset_sel_time = float(
                "nan"
            )  # set selection time to nan if not selecting subsets
