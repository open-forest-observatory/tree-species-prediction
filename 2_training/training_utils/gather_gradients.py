...
def gather_discriminator_gradients(
    temp_model, dataset, device, subbatch_size, batch_size
):
    """
    ...
    model_copy = TreeModel().to(device)
    ...
    subbatch_idxs = (
        []
    )  # store sample idxs of samples in subbatches relative to whole dset
    ...
    for batch_idxs in tqdm(
        chunk_list(all_idxs, batch_size),
        total=math.ceil(N / batch_size),
        desc="Gathering gradients for batches",
    ):
