...
def create_mmpretrain_structure(
    ...
    if missing_train:
        print(
            f"WARNING: {len(missing_train)} train datasets not found in labelled folder"
        )
    if missing_val:
    ...
    if flight_type != "both":
        combined_metadata = combined_metadata[
            combined_metadata["flight_type"] == flight_type
        ]
    ...
    if exclude_species:
        ...
        combined_metadata = combined_metadata[
            ~combined_metadata[species_col].isin(exclude_species)
        ]
    ...
    if replace_path is not None:
        train_data["image_path"] = train_data["image_path"].apply(
            lambda x: fix_image_path(x, data_folder_path, old_base_path)
        )
