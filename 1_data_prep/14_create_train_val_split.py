...
def create_species_quantity_matrix(trees_gdf, plot_ids, plot_weights, use_lumped=True):
    ...
    species_counts = trees_filtered.groupby(['plot_id', species_col]).size().reset_index(name='tree_count')
    
    # Pivot to create matrix: rows = plots, columns = species, values = tree counts
    species_matrix = species_counts.pivot(index='plot_id', columns=species_col, values='tree_count').fillna(0)
    ...
    print(f"Species quantity matrix: {len(species_matrix)} plots x {len(species_matrix.columns)} species")
    print(f"Matrix weighted by number of drone pairs per plot")
    
    return species_matrix


def find_best_split_with_trees(plots_gdf, species_matrix, plot_weights, val_frac=VAL_FRACTION, seed=SEED):
    """
...
def analyze_split_quality_with_trees(plots_gdf, species_matrix, plot_weights):
    ...
    print(f"Train weighted trees: {total_train_trees:,.0f} ({total_train_trees/(total_train_trees + total_val_trees):.1%})")
    print(f"Val weighted trees: {total_val_trees:,.0f} ({total_val_trees/(total_train_trees + total_val_trees):.1%})")
    ...
    comparison = comparison.loc[
        (comparison['Train_Weighted_Count'] + comparison['Val_Weighted_Count']).sort_values(ascending=False).index
    ]
    ...
    if len(abs_diffs) > 0:
        print(f"Largest species proportion difference: {abs_diffs.max():.3f} ({abs_diffs.idxmax()})")
...
def visualize_split_with_trees(plots_gdf, species_matrix, plot_weights, lump_level):
    ...
    train_gdf.plot(ax=ax1, color='green', alpha=0.7, edgecolor='darkgreen', linewidth=0.5)
    val_gdf.plot(ax=ax1, color='red', alpha=0.7, edgecolor='darkred', linewidth=0.5)
    
    ax1.set_title('Train-Val Split')
    train_patch = mpatches.Patch(color='green', label=f'Train ({len(train_gdf)} plots, {train_pairs} pairs)')
    val_patch = mpatches.Patch(color='red', label=f'Val ({len(val_gdf)} plots, {val_pairs} pairs)')
    ...
    fig_width = max(12, n_species * 0.8)  # At least 12 inches, or scale with species count
    fig2, ax2 = plt.subplots(figsize=(fig_width, 6))
    
    x = np.arange(n_species)
    width = 0.35
    
    bars_train = ax2.bar(x - width/2, train_props[all_species], width, label='Train', color='green', alpha=0.7)
    bars_val = ax2.bar(x + width/2, val_props[all_species], width, label='Val', color='red', alpha=0.7)
    
    # Label each bar with weighted counts
    for bar, species in zip(bars_train, all_species):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2, height + 0.005,
                    f"{int(train_species_total[species]):,}", ha='center', va='bottom',
                    fontsize=8, color='green', fontweight='bold', rotation=90)

    for bar, species in zip(bars_val, all_species):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2, height + 0.005,
                    f"{int(val_species_total[species]):,}", ha='center', va='bottom',
    ...
    ax2.set_title(f'Species Composition: Train vs Val ({lump_level} lumping)', fontsize=14)
    ax2.set_xticks(x)
...
if __name__ == "__main__":
    ...
    pairs_df = pd.read_csv("/ofo-share/scratch/amritha/tree-species-scratch/november-run/train_and_val_dsets.csv")
    ...
    print(f"Plots with multiple drone pairs: {sum(1 for c in plot_weights.values() if c > 1)}")
    ...
    trees_gdf, crosswalk = load_and_apply_crosswalk(trees_gdf, CROSSWALK_PATH, LUMP_LEVEL)
    ...
    plot_weights = {pid: weight for pid, weight in plot_weights.items() if pid in plots_with_trees}
    ...
    train_plot_ids, val_plot_ids = find_best_split_with_trees(plots_gdf, species_matrix, plot_weights)
    ...
    # Analyze results
    comparison = analyze_split_quality_with_trees(plots_gdf, species_matrix, plot_weights)
