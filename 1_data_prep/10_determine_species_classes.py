...
ground_drone_pairs = pd.read_csv(
    path_config.ground_plot_drone_mission_matches_file, dtype={"plot_id": str}
)
...
species_counts = (
    ground_trees.groupby("species_code")
    .agg(n_trees=("species_code", "size"), n_plots=("plot_id", "nunique"))
    .reset_index()
...
species_crosswalk["primary_species_l1"] = species_crosswalk["species_code"].isin(
    primary_species_l1
)
...
species_crosswalk["species_code_l1"] = species_crosswalk["species_code"].replace(
    mapping_l1
)
...
species_crosswalk["primary_species_l2"] = species_crosswalk["species_code"].isin(
    primary_species_l2
)
...
species_crosswalk["species_code_l2"] = species_crosswalk["species_code"].replace(
    mapping_l2
)
...
species_crosswalk["primary_species_l3"] = species_crosswalk["species_code"].isin(
    primary_species_l3
)
...
species_crosswalk["species_code_l3"] = species_crosswalk["species_code"].replace(
    mapping_l3
)
...
primary_species_l4 = primary_species_l3 + (
    "PI",
    "UMCA",
    "ALRU2",
    "CONU4",
...
species_crosswalk["primary_species_l4"] = species_crosswalk["species_code"].isin(
    primary_species_l4
)
...
species_crosswalk["species_code_l4"] = species_crosswalk["species_code"].replace(
    mapping_l4
)


# Write the result
species_crosswalk.to_csv(path_config.species_class_crosswalk_file, index=False)
