...
ORTHO_BASE_DIR = Path("/ofo-share/argo-data/argo-output/archive_20260202/species_project")
...
def crop_trees(ortho_path, vector_path, output_base_dir, dataset_name, species_mapping):
    ...
    with rasterio.open(ortho_path) as src:
        ...
        print(f"{dataset_name}: {successful} success, {failed} failed, {skipped} skipped")
        return successful, failed, skipped

def process_all_datasets(lumping_level, vector_dir, ortho_base_dir, split_csv, crosswalk_csv, output_base):
    ...
    total_stats = {'train': {'success': 0, 'failed': 0, 'skipped': 0, 'datasets': 0},
    ...
    for vector_path in vector_files:
        ...
        ortho_path = Path(ortho_base_dir) / dataset_name / "output" / f"{dataset_name}_ortho-dsm-ptcloud.tif"
        ...
        try:
            success, failed, skipped = crop_trees(str(ortho_path), str(vector_path), str(output_dir), dataset_name, species_mapping)
    ...
    for split in ['train', 'val', 'test']:
        ...
        print(f"{split}: {stats['datasets']} datasets, {stats['success']} trees cropped, {stats['failed']} failed, {stats['skipped']} skipped")
...
if __name__ == "__main__":
    ...
    process_all_datasets(lumping_level=LUMPING_LEVEL,
